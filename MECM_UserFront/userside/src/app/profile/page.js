...
export default function Page() {
  ...
  return (
    <NewLayout>
        ...
        <div className="border-8 cborder-blue-900 rounded-full p-2 my-6 shadow-xl "><img src={session?.user?.image} alt="" className="rounded-full w-36 h-36"/></div>

        <div className='flex flex-col m-4 p-2 shdw rounded-2xl'>
            {/* name */}
        {(true)?(<div className=' flex items-center  my-2 p-2 px-4 '>
            <div className="flex items-center"><label htmlFor="username" className='m-0 text-lg text-grey-400 font-bold'>Name: </label></div>
            <div className="flex items-center"><span name="username" className=' font-bold text-black text-lg'>{session?.user?.name}</span></div>
            
        </div>):(<></>)}

            {/* email */}
            {(true)?(<div className=' flex items-center  my-2 p-2 px-4 rounded-xl'>
            <div className="flex items-center"><label htmlFor="username" className='m-0 text-lg text-grey-400 font-bold'>Email: </label></div>
            <div className="flex items-center"><span name="useremail" className=' font-bold text-black text-lg'>{session?.user?.email}</span></div>
